...
local VERSION = "Lynx 20.10"
local nativeReboot = os.reboot
local nativeTerminal = term.native()
local ok, err = pcall(function(...)

if shell == nil then error("The Linux Kernel must be run from the shell.") end
if kernel ~= nil then error("The Linux Kernel cannot be run inside itself.") end
...
if not ok then
    ...
    printError(err .. "\nA critical error has occurred in the Linux Kernel, and the computer was left in an unstable state. A traceback has been saved to /var/logs/panic.log. The computer must restart to recover functionality. Press any key to reboot.")
    sleep(1)
    coroutine.yield("key")
    nativeReboot()
end
